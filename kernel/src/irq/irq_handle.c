...
void irq_handle(TrapFrame *tf)
{
	...
	{
		int irq_id = irq - 1000;
        panic("edi: %u, esi: %u, ebx: %u, edx: %u, ecx: %u, eax: %u, wrong irq_id: %u, error code: %u, eip: %u, cs: %u", tf->edi, tf->esi, tf->ebx, tf->edx, tf->ecx, tf->eax, irq_id, tf->error_code, tf->eip, tf->cs);
